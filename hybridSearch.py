...
for i, text in enumerate(texts):
    embedding = model.encode(text)  # encode the text, get single vector
    chunks.append({"chunk_id": i, "text": text, "embedding": embedding})

index = build_faiss_index(chunks)
bm25 = build_bm25_index(chunks)
# search
query = "What is in Nepal?"
query_embedding = model.encode(query) 
results = hybrid_search(index,bm25, query,query_embedding, chunks, top_k=2)
for r in results:
    print(r[1]["text"])